...
plot_psa <- ggplot(plotpsa.df, 
  ...
  labs(x="",y="PSA estimates", fill="Treatment")+
...
bnft_v2 <- ggplot(bnft2, aes(x = Label, y = Bnft, ymin=Bnft_lo, ymax=Bnft_hi)) +
  ...
  facet_wrap_custom(~type, scales = "free_x", scale_overrides = list(
    scale_override(1, scale_y_continuous(limits = c(-11,0),
                                         breaks = seq(from = -11, to = 0,by = 1),
                                         labels = seq(from = 11,  to = 0,by = -1), 
                     expand = c(0, 0))),
    scale_override(2, scale_y_continuous(limits = c(0, 11),
                                         breaks = seq(from = 0, to = 11,by = 1),
                                         labels = seq(from = 0,  to = 11,by = 1), 
                     expand = c(0, 0)))
  ))+
  coord_flip()+
  theme_bw() +
...
cpcty  <- data.frame(readxl::read_xlsx("../../OR decision model/Data/HIX_data.xlsx"))
key.df <- data.frame(readxl::read_xlsx("../../OR decision model/Data/Sleutellijst_hix_onzedata.xlsx"))
