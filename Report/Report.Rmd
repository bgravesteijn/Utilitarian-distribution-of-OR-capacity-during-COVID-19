...
# Section 2: The model
In this section we describe some of the steps that are done to get the results. We refer to several functions that help to run the model. These files can be found in the R folder on GitHubbut are not shown in full detail. This report focusses mainly on the results. However, we enourage those interested to take a look at the code, replicate the results or modify the model for you own needs. 
...
plotpsa.df$tx <- ifelse(plotpsa.df$Param%in%c("Surv_no_tx","QoL_no_tx"),"Without treatment",
                        ifelse(plotpsa.df$Param%in%c("Surv_tx","QoL_tx"),"With treatment",
                               "Either"))
plotpsa.df$tx <- factor(plotpsa.df$tx)
plotpsa.df$tx <- factor(plotpsa.df$tx, levels = c("Without treatment", "With treatment","Either"))
...
#Make the plot of the parametes values as used in the PSA dataset
...
# set on TRUE to store the figures in the correct folder for the revisio
ggsave(plot_psa, 
...
bnft$Intervention <- as.character(df_res$Intervention[seq(from = 1, to = length(df_res$Intervention), by= length(unique(param_psa$iter)) * length(unique(df_res$delay)))])
...
for(p in unique(bnft$Label)){
  ...
  # Calculate benefit
  bnft[bnft$Label==p&bnft$type=="DALY","Bnft"] <- quantile(DALY2-DALY999, probs=0.5)
  
  bnft[bnft$Label==p&bnft$type=="DALY","Bnft_lo"] <- quantile(DALY2-DALY999, probs=0.025)
  
  bnft[bnft$Label==p&bnft$type=="DALY","Bnft_hi"] <- quantile(DALY2-DALY999, probs=0.975)
  ...
  # Calculate benefit
  bnft[bnft$Label==p&bnft$type=="YLL","Bnft"]    <- quantile(YLL2-YLL999, probs=0.5)
  bnft[bnft$Label==p&bnft$type=="YLL","Bnft_lo"] <- quantile(YLL2-YLL999, probs=0.025)
  bnft[bnft$Label==p&bnft$type=="YLL","Bnft_hi"] <- quantile(YLL2-YLL999, probs=0.975)
 }
...
urg$Intervention <- as.character( df_res$Intervention[seq(from = 1, to = length(df_res$Intervention), by= length(unique(param_psa$iter)) * length(unique(df_res$delay)))])
...
for(p in unique(urg$Label)){
  ...
  # Calculate slope
  urg[urg$Label==p&urg$type=="DALY","urg"] <- quantile(DALY2-DALY52, probs=0.5)/50*52/12
  urg[urg$Label==p&urg$type=="DALY","urg_lo"] <- quantile(DALY2-DALY52, probs=0.025)/50*52/12
  urg[urg$Label==p&urg$type=="DALY","urg_hi"] <- quantile(DALY2-DALY52, probs=0.975)/50*52/12
  
  # YLLs with minimal delay
  YLL2   <- df_res[df_res$Label==p&df_res$delay==2,]$YLL
  ...
  # Calculate benefit
  urg[urg$Label==p&urg$type=="YLL","urg"]    <- quantile(YLL2-YLL52, probs=0.5)/50*52/12
  urg[urg$Label==p&urg$type=="YLL","urg_lo"] <- quantile(YLL2-YLL52, probs=0.025)/50*52/12
  urg[urg$Label==p&urg$type=="YLL","urg_hi"] <- quantile(YLL2-YLL52, probs=0.975)/50*52/12
}
...
plot_descend_steaper <- ggplot(desc_df_steaper, 
  ...
  geom_line(cex = 2) + theme_bw() + theme(legend.position = "bottom", text=element_text(size=18))+
