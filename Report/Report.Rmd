...
date: "Report created: `r format(Sys.time(), '%d %B, %Y')`, Last update model: 23/6/2020"
...
p_load("here", "dplyr", "devtools", "scales", "ellipse", "ggplot2", "lazyeval", "igraph", "ggraph", "reshape2", "knitr", "citr", "plyr", "stats", "diagram", "EnvStats", "data.table", "gridExtra","ggrepel","tableone", "tidyr")   
...
source("../R/modify facet axes.R") # Code to change the axis of facets in ggplots
```
...
The first sections gives a summary of the research. It includes the most important conclussions and gives an overview of the model assumptions. In the second section we describe the model in more detail and provide an explanation of the files used to generate the results. All the code and summary data is provided on GitHub and can be used to replicate the results or modify the model for a context. In the final section, section 3, all model results are included. 
...
- For some disease, surgery delay can cause an irreversible QoL decrease (e.g. getting blind, permanent function loss after fracture). 


# Section 2: The model
In this section we describe some of the steps that are done to get the results. We refer to several functions that help to run the model. These files can be found in the R folder on GitHub are not shown in full detail. This report focuses mainly on the results. However, we encourage those interested to take a look at the code, replicate the results or modify the model for you own needs. 
...
We import the the parameters found in the literature for the different populations and different interventions. These parameters are stored in the `Model parameters.xlsx` file in the _data_ folder and a list of reference can be found in Appendix A of the manuscript.
...
We make use of a probabilistic sensitivity analysis (PSA) to demonstrate the robustness of the model outcomes. In other words, how sensitive are our model outcomes to the uncertainty of the input parameters. A PSA requires a dataset which contains a set of parameter values drawn from the distribution for each PSA iteration. 
...
#Add units
units <- data.frame(Param = c("Age", 
                              "QoL_no_tx", "QoL_tx", 
                              "Surv_no_tx", "Surv_tx",
                              "Tx_eff",
                              "Time_noeff_Surv", "Time_noeff_QoL"),
                     Unit = c("Years", 
                             "Utility", "Utility", 
                             "Probability per year", "Probability per year", 
                             "OR/HR/RR", 
                             "Weeks", "weeks"))
...
write.csv2(param_psa_summary, file = "../figures/main_results/res_psa.csv")
```

The following figures show the input parameters of all diseases in forest plots.
```{r, echo = FALSE, fig.width = 25, fig.height = 25}
plotpsa.df <- param_psa[!param_psa$Param %in% c("Time_noeff_QoL", "Tx_eff"), ]

#adjust so that notx and tx are different
plotpsa.df$tx <- ifelse(plotpsa.df$Param %in% c("Surv_no_tx", "QoL_no_tx"),
                        "Without treatment",
                        ifelse(plotpsa.df$Param %in% c("Surv_tx","QoL_tx"),
                               "With treatment",
                               "Either"))

plotpsa.df$tx <- factor(plotpsa.df$tx)
plotpsa.df$tx <- factor(plotpsa.df$tx, levels = c("Without treatment", 
                                                  "With treatment", 
                                                  "Either"))

plotpsa.df$Param <- factor(plotpsa.df$Param)
levels(plotpsa.df$Param) <- c("Age", "QoL", "QoL", "Survival", "Survival", "Time untill no Survival effect")

plotpsa.df$Label <- factor(plotpsa.df$Label)
plotpsa.df$Label <- factor(plotpsa.df$Label, levels = rev(levels(plotpsa.df$Label)))

#Make the plot of the parameters values as used in the PSA dataset
plot_psa <- ggplot(plotpsa.df, 
                       aes(x = Label, y = psa_est, fill = tx)) +
  geom_boxplot(alpha = 0.8) +
  labs(x = "", y = "Distribution of parameter value for PSA", fill = "Treatment") +
  facet_wrap(~ Param, scales = "free_x", nrow = 1) +
  scale_y_log10() +
  coord_flip() +
  theme_bw() +
  theme(text = element_text(size = 15)) +
  scale_fill_manual(values = c("salmon2", "seagreen", "Grey")) +
...
ggsave(plot_psa, 
       device = "png", file = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Corrections/Figurestables/fig2.png", 
       width = 50, height = 35, units = "cm")
```

```{r, eval = FALSE, echo = FALSE, fig.width = 25, fig.height = 25}
# set on TRUE to store the figures in the correct folder for the revision
ggsave(plot_psa, 
       device = "png", file = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Fig2.png", 
       width = 50, height = 35, units = "cm")
```
...
This means that the transition matrix is different at different cycles during the simulation. Therefore, we make a transtion probabilty matrix for every cycle. In more technical terms, we add an extra dimension to the 2x2 matrix, a dimension of the number of cycles. This results in an array of transition probabilities, with dimensions `n_states` x `n_states` x `n_cycles`. While running the model the transition probability matrix corresponding to the current cycle is used to model what happends to the cohort. 

Part of the `make_m_trans` function incluces adding the national age specific mortality. This age specific probability to die is added to the age specific survival propability. Since the time frame of the model is just one year. The cohort is not againg, in years, during the simulation. 

An other part of the `make_m_trans` function incorporates the "time to no effect on the survival". This is a parameter that describes the time after which surgery does not inprove the survival of a patient anymore. For example, the tumor doubling time. After some time, the srugery might inprove the quality of life of the patient by removing the tumor, but does not extent the life time of the patient anymore. The code checks for every surgery if this time should be incorrportated. If so, the transition probability to die in the post-operative state will be set equal to the survival probability without the surgery. This means that even after surgery, the patient will be faces the same risk of dying due to the disease as it was facing before the surgery.  
...
bnft$Pop          <- as.character(unique(df_res$Pop))
bnft$Intervention <- as.character(df_res$Intervention[seq(from = 1, 
                                                          to = length(df_res$Intervention),
                                                          by = length(unique(param_psa$iter)) * length(unique(df_res$delay)))])

for(p in unique(bnft$Label)){
  # DALYs with minimal delay = 2 weeks
  DALY2   <- df_res[df_res$Label == p & df_res$delay == 2, ]$DALY
  
  # DALYs without surgery
  DALY999 <- df_res[df_res$Label == p & df_res$delay == 999, ]$DALY
  
  # Calculate benefit
  bnft[bnft$Label == p & bnft$type == "DALY", "Bnft"]    <- quantile(DALY2 - DALY999, 
                                                                  probs = 0.5)
  
  bnft[bnft$Label == p & bnft$type == "DALY", "Bnft_lo"] <- quantile(DALY2 - DALY999,
                                                                     probs = 0.025)
  
  bnft[bnft$Label == p & bnft$type == "DALY", "Bnft_hi"] <- quantile(DALY2 - DALY999, 
                                                              probs = 0.975)
  
  # LYs with minimal delay
  YLL2   <- df_res[df_res$Label == p & df_res$delay == 2, ]$YLL
  
  # LYs without surgery
  YLL999 <- df_res[df_res$Label == p & df_res$delay == 999, ]$YLL
  
  # Calculate benefit
  bnft[bnft$Label == p & bnft$type == "YLL", "Bnft"]    <- quantile(YLL2 - YLL999,
                                                                    probs = 0.5)
  bnft[bnft$Label == p & bnft$type == "YLL", "Bnft_lo"] <- quantile(YLL2 - YLL999,
                                                                    probs = 0.025)
  bnft[bnft$Label == p & bnft$type == "YLL", "Bnft_hi"] <- quantile(YLL2 - YLL999,
                                                                    probs = 0.975)
 }

# Urgency
urg <- expand.grid(Label = as.character(unique(df_res$Label)),
                     urg = NA,
                  urg_lo = NA,
                  urg_hi = NA,
                   type  = c("DALY", "YLL"))
...
urg$Pop          <- as.character(unique(df_res$Pop))
urg$Intervention <- as.character( df_res$Intervention[seq(from = 1, 
                                                          to = length(df_res$Intervention),
                                                          by = length(unique(param_psa$iter)) * length(unique(df_res$delay)))])

for(p in unique(urg$Label)){
  # DALYs with minimal delay
  DALY2   <- df_res[df_res$Label == p & df_res$delay == 2, ]$DALY
  
  # DALYs with maximal delay
  DALY52 <- df_res[df_res$Label == p & df_res$delay == 52, ]$DALY
  
  # Calculate slope
  urg[urg$Label == p & urg$type == "DALY", "urg"] <- quantile(DALY2 - DALY52,
                                                              probs = 0.5)/50*52/12
  urg[urg$Label == p & urg$type == "DALY", "urg_lo"] <- quantile(DALY2 - DALY52,
                                                                 probs = 0.025)/50*52/12
  urg[urg$Label == p & urg$type == "DALY", "urg_hi"] <- quantile(DALY2 - DALY52,
                                                                 probs = 0.975)/50*52/12
  
  # YLLs with minimal delay, 2 weeks
  YLL2   <- df_res[df_res$Label == p & df_res$delay == 2, ]$YLL
  
  # YLLs with maximal delay, 52 weeks
  YLL52 <- df_res[df_res$Label == p & df_res$delay == 52, ]$YLL
  
  # Calculate benefit
  urg[urg$Label == p & urg$type == "YLL", "urg"]    <- quantile(YLL2 - YLL52,
                                                                probs = 0.5)/50*52/12
  urg[urg$Label == p & urg$type == "YLL", "urg_lo"] <- quantile(YLL2 - YLL52,
                                                                probs = 0.025)/50*52/12
  urg[urg$Label == p & urg$type == "YLL", "urg_hi"] <- quantile(YLL2 - YLL52,
                                                                probs = 0.975)/50*52/12
}

#Extract the right order of presentation
urg_DALY <- urg[urg$type == "DALY", ]
order_populations   <- as.character(urg_DALY$Label[order(urg_DALY$urg, decreasing = FALSE)]) # using the populations (patient & intervention)
order_patients      <- as.character(urg_DALY$Pop[order(urg_DALY$urg, decreasing = FALSE)]) # of patients 
order_interventions <- as.character(urg_DALY$Intervention[order(urg_DALY$urg, decreasing = FALSE)]) # of interventions
```

## Main results
```{r, echo=FALSE, message=FALSE}
# df_results_pooled$std <- 0
# for(i in unique(df_results_pooled$Label)){
#   df_results_pooled$std[df_results_pooled$Label == i] <- 
#     mean(df_results_pooled$QALY_med[df_results_pooled$Label == i])
# }

desc_df <- results_pooled[results_pooled$delay != 999, ]

desc_df_steaper <- desc_df[desc_df$Label%in%c("Instable AP, CABG",
                                              "AVR, TAVI",
                                              "Carotid endarterectomy"),]

desc_df_flatter <- desc_df[desc_df$Label%in%c("ASD, repair",
                                              "NSCLC, lobectomy",
                                              "UUT Ca., nefroureterectomy",
                                              "Pancreasca., Whipple",
                                              "ESHF, LVAD",
                                              "Mild larynx ca., resection",
                                              "Ovarium ca., resection and HIPEC", 
                                              "High-risk endometrium ca, resection",
                                              "Mammaca., mastectomy", 
                                              "MI bladder ca, cystectomy",
                                              "High-grade glioma, resection",
                                              "PAD F3-4, bypass",
                                              "Liver metastasis Colon cancer, resection",
                                              "Penis cancer, resection",
                                              "MI bladder cancer, cystectomy",
                                              "Pancreas cancer, Whipple",
                                              "HCC, resection",
                                              "Mild larynx cancer, resection",
                                              "Penis ca., resection",
                                              "Ovarium cancer, resection and HIPEC",
                                              "Liver metastasis colonca, resection"), ]

desc_df_lin     <- desc_df[!desc_df$Label %in% c(desc_df_flatter$Label, desc_df_steaper$Label), ]

plot_descend_steaper <- ggplot(desc_df_steaper, 
       aes(x = delay, y = DALY_med, col = Label)) +
  geom_line(cex = 2) + 
  theme_bw() + 
  theme(legend.position = "bottom", text=element_text(size = 18)) +
  geom_vline(xintercept = seq(12, 42, by = 10), lty = 2) +
  labs(x = "Delay (weeks)", y = "DALYs", col = "Procedure")

plot_descend_steaper

plot_descend_flatter <- ggplot(desc_df_flatter, 
       aes(x = delay, y = DALY_med, col = Label)) +
  geom_line(cex = 2) + 
  theme_bw() + 
  theme(legend.position = "bottom", text=element_text(size = 18)) +
...
plot_descend_flatter

plot_descend_lin <- ggplot(desc_df_lin, 
       aes(x = delay, y = DALY_med, col = Label)) +
  geom_line(cex = 2) +
  theme_bw() +
  theme(legend.position = "bottom", text = element_text(size = 18)) +
...
# Save the figures 
ggsave(plot_descend_steaper, filename = "../figures/main_results/descend_steiler.png", device = "png", 
       width = 15, height = 10)
ggsave(plot_descend_flatter, filename = "../figures/main_results/descend_vlakker.png", device = "png", 
       width = 15, height = 10)
ggsave(plot_descend_lin, filename = "../figures/main_results/descend_lin.png", device = "png", 
       width = 15, height = 10)
```
...
est_ci_DALY <- matrix(nrow = length(order_populations),
                      ncol = length(unique(results_pooled$delay)),
                      dimnames = list(order_populations, 
                                      unique(results_pooled$delay)))
# Loop over the diseases, and store the DALYs (est+95% CI) in the matrix
for(i in order_populations){
est_ci_DALY[i,] <- paste(sprintf("%.2f",results_pooled$DALY_med[results_pooled$Label==i]),
                         " (",
                         sprintf("%.2f",results_pooled$DALY_lo[results_pooled$Label==i]),
                         " - ",
                         sprintf("%.2f",results_pooled$DALY_hi[results_pooled$Label==i]),
                         ")", sep=""
                         )
...
res_main <- data.frame(est_ci_DALY)

#Make the colnames prettier
colnames(res_main) <- paste("DALY, delay =",unique(results_pooled$delay),"weeks")

# To store the urg in
res_main$`DALY/month` <- NA

# Obtain the AAC/max (estimate +95% CI) and store in the data.frame
for(i in order_populations){
res_main[i,"DALY/month"] <- paste(sprintf("%.2f",
                                             -urg_DALY$urg[
                                               urg_DALY$Label==i]
                                             ),
                                     " (",
...
knitr::kable(res_main[, -1], font_size = 8)  # Print the results 

write.csv2(res_main[, -1], file="../figures/main_results/main_results.csv")
```
...
#Descending order 
bnft$Label <- factor(bnft$Label, levels = rev(order_populations))
bnft$type  <- factor(bnft$type, levels = c("DALY", "YLL"))
plot_main <- ggplot(bnft, 
                    aes(x = Label, y = Bnft, ymin = Bnft_lo, ymax = Bnft_hi))+
  geom_pointrange() +
  labs(x = "Population/intervention", y = "Associated with no surgery") +
  theme_bw() +
...
```{r}
bnft2 <- bnft
bnft2$Label <- factor(bnft2$Label, levels = rev(order_populations))

## part of DALY that is LY


bnft2$Bnft   [bnft2$type == "YLL"] <- -bnft2$Bnft   [bnft2$type == "YLL"]
bnft2$Bnft_lo[bnft2$type == "YLL"] <- -bnft2$Bnft_lo[bnft2$type == "YLL"]
bnft2$Bnft_hi[bnft2$type == "YLL"] <- -bnft2$Bnft_hi[bnft2$type == "YLL"]

bnft2[bnft2$type == "DALY", c("Bnft", "Bnft_lo", "Bnft_hi")][bnft2[bnft2$type == "DALY", c("Bnft", "Bnft_lo", "Bnft_hi")] > 0] <- 0
bnft2[bnft2$type == "YLL",  c("Bnft", "Bnft_lo", "Bnft_hi")][bnft2[bnft2$type == "YLL",  c("Bnft", "Bnft_lo", "Bnft_hi")] < 0] <- 0

# NOTE: The limit of the breaks might be better coded using the rounded value of the Bnft_hi value. 
breaks <- -15:15
names(breaks) <- abs(breaks)
bnft2$type <- factor(bnft2$type, levels = c("DALY","YLL"))

bnft_v2 <- ggplot(bnft2, aes(x = Label, y = Bnft, ymin = Bnft_lo, ymax = Bnft_hi )) +
  geom_col(alpha = 0.6) +
  geom_linerange()+
  geom_hline(yintercept = 0, colour = "black") +
  facet_wrap_custom(~type, scales = "free_x", scale_overrides = list(
    scale_override(1, scale_y_continuous(limits = c(-15,0),
                                         breaks = seq(from = -15, to = 0, by = 1),
                                         labels = seq(from = 15,  to = 0, by = -1), 
                     expand = c(0, 0))),
    scale_override(2, scale_y_continuous(limits = c(0, 15),
                                         breaks = seq(from = 0, to = 15, by = 1),
                                         labels = seq(from = 0, to = 15, by = 1), 
                     expand = c(0, 0)))
  ))+
  coord_flip()+
  theme_bw() +
  ...
  labs(y = "Associated with no surgery", x = "Surgery")

bnft_v2
ggsave(bnft_v2,filename = "../figures/main_results/bnft_v2.png", width = 15, height = 10)

# ggsave(bnft_v2,filename = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Corrections/Figurestables/fig3.png", width = 15, height = 10)

```

```{r, eval = FALSE}
# Set to true to store the figure in the 
ggsave(bnft_v2,filename = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Fig_appendixB.png", width = 15, height = 10)
...
urg$Label <- factor(urg$Label, levels = rev(order_populations))

plot_main <- ggplot(urg, 
                    aes(x = Label, 
                        y = urg,     
                        ymin = urg_lo, 
                        ymax = urg_hi)) +
  geom_pointrange()+
  labs(x = "Population/intervention", y = "Loss per month delay")+
  theme_bw()+
  theme(text = element_text(size = 20))+
  coord_flip() +
  facet_wrap(~type)
...
ggsave(plot_main, filename = "../figures/main_results/loss_pw.png", device = "png", width = 15, height = 15)
```


```{r}
# Make an new urg df, which we can modify
urg2 <- urg

#Make sure the procedures are ordered in descending order
urg2$Label <- factor(urg2$Label, levels = rev(order_populations))

## YLLs are left, so need to be negative
urg2$urg[   urg2$type == "YLL"] <- -urg2$urg[   urg2$type == "YLL"]
urg2$urg_lo[urg2$type == "YLL"] <- -urg2$urg_lo[urg2$type == "YLL"]
urg2$urg_hi[urg2$type == "YLL"] <- -urg2$urg_hi[urg2$type == "YLL"]

## Do not allow negative values (make them 0)
urg2[urg2$type == "DALY", c("urg", "urg_lo", "urg_hi")][urg2[urg2$type == "DALY", c("urg", "urg_lo", "urg_hi")] > 0] <- 0
urg2[urg2$type == "YLL",  c("urg", "urg_lo", "urg_hi")][urg2[urg2$type == "YLL",  c("urg", "urg_lo", "urg_hi")] < 0] <- 0

## To ensure that DALYs also have positive ticks (x-axis)
breaks <- seq(-0.4, 0.4, by = 0.05)
names(breaks) <- abs(breaks)

## To ensure that DALY = Left, and LY = right
urg2$type <- factor(urg2$type, levels = c("DALY", "YLL"))

#Build plot
urg_v2 <- ggplot(urg2, aes(x = Label, y = urg, ymin = urg_lo, ymax = urg_hi)) +
  geom_col(alpha = 0.6) +
  geom_linerange() +
  geom_hline(yintercept = 0, colour = "black") +
  
  facet_wrap_custom(~type, scales = "free_x", scale_overrides = list(
    scale_override(1, scale_y_continuous(limits = c(-0.25, 0),
                                         breaks = seq(from = -0.25, to = 0,by = 0.05),
                                         labels = seq(from = 0.25,  to = 0,by = -0.05), 
                     expand = c(0, 0))),
    scale_override(2, scale_y_continuous(limits = c(0, 0.25),
                                         breaks = seq(from = 0, to = 0.25,by = 0.05),
                                         labels = seq(from = 0, to = 0.25,by = 0.05), 
                     expand = c(0, 0)))
  ))+
  coord_flip() +
  theme_bw() +
  theme(panel.spacing.x = unit(0, "pt"), 
        strip.background = element_rect(colour = "black"),
        text = element_text(size = 18))+
  labs(y = "Per month delay", x = "Surgery")

urg_v2
ggsave(urg_v2,filename = "../figures/main_results/urg_v2.png", width = 15, height = 10)

ggsave(urg_v2,filename = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Corrections/Figurestables/fig3.png", width = 15, height = 10)

#ggsave(urg_v2,filename = "../../OR-decision-model/Manuscript/Submissions/Value in Health/Revision 1/Fig3.png", width = 15, height = 10)
...
# DALY
all_images_DALY <- vector(mode = "character" ,length = length(order_populations))
order_populations <- as.vector(order_populations)
order_patients    <- as.vector(order_patients)

for(i in 1:length(order_populations)){
  p <- order_populations[i]
  p <- gsub(" ", "_", p)
 all_images_DALY[i] <- (paste("../figures/QALY_per_pop/", p, "_DALY.png", sep = ""))
}
...
param_psa_summary$Population <- param_psa$Population[1:length(param_psa_summary$Label)]

### IF we did not create the PSA files already, plot them now
if(plot_psa_again){
## Save psa data into figures
for(d in unique(urg$Label)){
  p <- gsub(' ', "_", d)
...
# USe the names of the population rather than the combination of patients and resection
if(plot_psa_again){
## Save psa data into figures
for(d in order_patients){
  p <- gsub(' ', "_", d)
  png(paste("../figures/QALY_per_pop/",p,"_psa.png",sep=""), width = 1200, height=400 )
  gridExtra::grid.table(param_psa_summary[param_psa_summary$Population==d,])
  dev.off()
}
}
...
all_images_m <- matrix(c(all_psa, all_images_DALY), ncol = 2, byrow = FALSE) #in a matrix
...
cpcty$duration_or_lo[cpcty$Hix_label == "Niertransplantatie traj ontv"] <- cpcty$duration_or_lo[cpcty$Hix_label == "Niertransplantatie traj donor"] + cpcty$duration_or_lo[cpcty$Hix_label == "Niertransplantatie traj ontv"]

cpcty$duration_or_med[cpcty$Hix_label == "Niertransplantatie traj ontv"] <- cpcty$duration_or_med[cpcty$Hix_label == "Niertransplantatie traj donor"] + cpcty$duration_or_med[cpcty$Hix_label == "Niertransplantatie traj ontv"]

cpcty$duration_or_hi[cpcty$Hix_label == "Niertransplantatie traj ontv"] <- cpcty$duration_or_hi[cpcty$Hix_label == "Niertransplantatie traj donor"] + cpcty$duration_or_hi[cpcty$Hix_label == "Niertransplantatie traj ontv"]

cpcty$duration_or_lo[cpcty$Hix_label == "Levertranspl traj ontv"] <-  cpcty$duration_or_lo[cpcty$Hix_label == "Part levertranspl traj donor"] + cpcty$duration_or_lo[cpcty$Hix_label == "Levertranspl traj ontv"]

cpcty$duration_or_med[cpcty$Hix_label == "Levertranspl traj ontv"] <-  cpcty$duration_or_med[cpcty$Hix_label == "Part levertranspl traj donor"] + cpcty$duration_or_med[cpcty$Hix_label == "Levertranspl traj ontv"]

cpcty$duration_or_hi[cpcty$Hix_label == "Levertranspl traj ontv"] <-  cpcty$duration_or_hi[cpcty$Hix_label == "Part levertranspl traj donor"] + cpcty$duration_or_hi[cpcty$Hix_label == "Levertranspl traj ontv"]
...
df_res_main <- merge(urg, bnft, by = c("Label", "type", "Pop", "Intervention")) 
...
save(df_res_main, file = "../output/df_res_main.Rdata")
```
...
urg_cpcty <- ggplot(df_res_main[df_res_main$type=="QALY",],
                  aes(x=los_nonicu_med, y=duration_or_med, label=Label,
                      col=urg))+
  geom_label_repel(box.padding   = 0.40, 
                  point.padding = 0.5,
...
```{r,echo = FALSE, fig.width=16, fig.height=16}
urg_cpcty2 <- ggplot(df_res_main[df_res_main$type=="QALY"&df_res_main$Label!="ESLD, transplant",],
                  aes(x=duration_or_med, y=urg, label=Label))+
  geom_label_repel(box.padding   = 0.40, 
                  point.padding = 0.5,
                  segment.color = 'grey50',cex=6)+
  geom_point(cex=3)+
  theme_bw()+
  labs(x="Duration surgery (min)",
       y="QALY loss per month")+
  theme(text=element_text(size=20))
       
urg_cpcty2

ggsave(urg_cpcty2, file="../figures/main_results/duration_urg_v2.png", width=40, height=30, units = "cm")
```
...
```{r correlation between rankings}
# % of total hospital capacity
sum(cpcty$count[cpcty$Hix_label%in%key.df$Hix_label_1])/sum(cpcty$count)*100

bnft[bnft$Bnft == min(bnft$Bnft[bnft$type == "QALY"]), ]
bnft[bnft$Bnft == max(bnft$Bnft[bnft$type == "QALY"]), ]

bnft_rnk_ly       <- bnft[bnft$type == "LY",]
bnft_rnk_ly       <- bnft_rnk_ly[order(bnft_rnk_ly$Bnft), ]
bnft_rnk_ly$rnk   <- nrow(bnft_rnk_ly):1
bnft_rnk_qaly     <- bnft[bnft$type == "QALY",]
bnft_rnk_qaly     <- bnft_rnk_qaly[order(bnft_rnk_qaly$Bnft), ]
bnft_rnk_qaly$rnk <- nrow(bnft_rnk_ly):1
urg_rnk_ly        <- urg[urg$type == "LY",]
urg_rnk_ly        <- urg_rnk_ly[order(urg_rnk_ly$urg), ]
urg_rnk_ly$rnk    <- nrow(bnft_rnk_ly):1
urg_rnk_qaly      <- urg[urg$type == "QALY", ]
urg_rnk_qaly      <- urg_rnk_qaly[order(urg_rnk_qaly$urg), ]
urg_rnk_qaly$rnk  <- nrow(bnft_rnk_ly):1

bnft_lyqaly_cor <- merge(bnft_rnk_ly, bnft_rnk_qaly, by = "Label")
cor.test(bnft_lyqaly_cor$rnk.y,
         bnft_lyqaly_cor$rnk.x, method = "spearman")

bnfturg_qaly_cor <- merge(bnft_rnk_qaly, urg_rnk_qaly, by = "Label")
cor.test(bnfturg_qaly_cor$rnk.y,
         bnfturg_qaly_cor$rnk.x, method = "spearman")

ly_urg_qaly_cor <- merge(bnft_rnk_ly, urg_rnk_qaly, by = "Label")
cor.test(ly_urg_qaly_cor$rnk.y,
         ly_urg_qaly_cor$rnk.x, method = "spearman")

urg_lyqaly_compare <- merge(urg_rnk_qaly, urg_rnk_ly, by = "Label")
cor_all            <- data.frame(type = rep(c("urg_qaly", "urg_ly", "bnft_qaly", "bnft_ly"),
                               each = nrow(bnft_rnk_qaly)),
                      label = c(as.character(urg_rnk_qaly$Label),
                                as.character(urg_rnk_ly$Label),
                                as.character(bnft_rnk_qaly$Label),
                                as.character(bnft_rnk_ly$Label)),
                      rank = c(urg_rnk_qaly$rnk,
                              urg_rnk_ly$rnk,
                              bnft_rnk_qaly$rnk,
                              bnft_rnk_ly$rnk))
cor_all <- reshape(data = cor_all, direction = "wide", 
        idvar = "label", 
        timevar =  "type" )
library(corrplot)
colnames(cor_all)[-1] <- c("Urgency - QALY", "Urgency - LY","Benefit - QALY","Benefit - LY")
PerformanceAnalytics::chart.Correlation(cor_all[, -1], method = "spearman")

tiff("../figures/main_results/correlation_rankings.tiff", res = 600, width = 4000, height = 4000, compression = "lzw")
PerformanceAnalytics::chart.Correlation(cor_all[, -1], method = "spearman")
dev.off()

rnk_ly_min_qaly <- urg_lyqaly_compare$rnk.y - urg_lyqaly_compare$rnk.x
urg_lyqaly_compare[which(rnk_ly_min_qaly == min(rnk_ly_min_qaly)), c("Label", "rnk.x", "rnk.y")]
urg_lyqaly_compare[which(rnk_ly_min_qaly == max(rnk_ly_min_qaly)), c("Label", "rnk.x", "rnk.y")]
```

```{r class evidence}
library(tableone)
param <- readxl::read_excel("../data/Model parameters.xlsx", sheet = "Complete")

param <- data.table(param)
param <- param[!is.na(param$Unit), ]

tbl_cls <- CreateTableOne(vars   = c("Evidence_type", "Evidence_class"),
                          strata = "Param",
                          data   = param, 
                          test   = FALSE,
                          factorVars = c("Evidence_type", "Evidence_class"))

tbl_cls2 <- print(tbl_cls, printToggle = FALSE)  
  
kableone(tbl_cls)
write.csv2(tbl_cls2, file = "../figures/main_results/evidence_tbl.csv")
```


# Make code for the "integraal capaciteits management" group
```{r}
load("../output/psa_pooled.Rdata")
delay <- unique(df_results_pooled$delay)
df_icm <- expand.grid(  PopID = NA,
                        Pop = unique(df_results_pooled$Label),  # Disease name 
                        QALY_med = NA,
                        QALY_lo = NA,
                        QALY_hi = NA,
                        bnft_med = NA,
...
n_weeks_loss <- paste("loss", 1:52, sep = "") 
df_icm <-  cbind(df_icm, setNames(lapply(n_weeks_loss, function(x) x = NA), n_weeks_loss))
df_icm$PopID <- c(1:length(df_icm$Pop)) # Add a population ID
head(df_icm)


if(sum(unique(df_results_pooled$Label) == bnft$Label)/length(bnft$Label) == 1){
df_icm$QALY_med <- df_results_pooled$QALY_med[df_results_pooled$delay == 2]
df_icm$QALY_lo  <- df_results_pooled$QALY_lo[ df_results_pooled$delay == 2]
df_icm$QALY_hi  <- df_results_pooled$QALY_hi[ df_results_pooled$delay == 2]

df_icm$bnft_med <- bnft$Bnft[   bnft$type == "QALY"]
df_icm$bnft_lo  <- bnft$Bnft_lo[bnft$type == "QALY"]
df_icm$bnft_hi  <- bnft$Bnft_hi[bnft$type == "QALY"]
}


loss_icm <- expand.grid(Label = as.character(unique(df_res$Label)),
                        loss = NA,
                        loss_lo = NA,
                        loss_hi = NA,
                        type    = "QALY")
...
for(t in 1:(length(delay)-2)){
  ...
  QALY_t1 <- df_res[df_res$Label == p & df_res$delay == t1, ]$QALY
  
  # QALYs with maximal delay
  QALY_t2 <- df_res[df_res$Label == p & df_res$delay == t2, ]$QALY
  
  # Calculate slope
  loss_icm[loss_icm$Label == p & loss_icm$type == "QALY", "loss"]    <- quantile(QALY_t1 - QALY_t2, probs = 0.5  )/delta_t
  loss_icm[loss_icm$Label == p & loss_icm$type == "QALY", "loss_lo"] <- quantile(QALY_t1 - QALY_t2, probs = 0.025)/delta_t
  loss_icm[loss_icm$Label == p & loss_icm$type == "QALY", "loss_hi"] <- quantile(QALY_t1 - QALY_t2, probs = 0.975)/delta_t
  
  df_icm[, (n_weeks_loss[t1:t2])] <- loss_icm$loss # Only store the mean QALY loss
  }
}


df_icm[ , -which(names(df_icm) %in% c("loss1"))] # Remove the column with the loss of the first week as loss week 2 is considered surgery now
...
keycol <- "Week"
valuecol <- "Value"
gathercols <- c(paste("loss", 2:52, sep = ""))

df_icm_long <- gather_(olddata_wide, keycol, valuecol, gathercols)
df_icm_long <- df_icm_long[order(df_icm_long$Pop), ] # Order the data 

# Save the file 
write.csv(df_icm, file = "../output/df_icm.csv")
write.csv(df_icm, file = "../output/df_icm_long.csv")

library("xlsx")
# Write the first data set in a new workbook
write.xlsx(df_icm, file = "../output/df_icm.xlsx",
      sheetName = "QALY and loss", append = FALSE)

write.xlsx(df_icm, file = "../output/df_icm_long.xlsx",
      sheetName = "QALY and loss", append = FALSE)
```
...
- The research team: 
Benjamin Gravesteijn, Eline Krijkamp, Jan Busschbach, Geert Geleijnse,  Isabel Retel Helmrich, Sophie Bruinsma, Céline van Lint, Ernest van Veen,  Ewout Steyerberg, Kees Verhoef, Jan van Saase, Hester Lingsma, Rob Baatenburg de Jong and collaborations**

** We would like to acknowledge our collaborations Chris Bangma, Ivo Beetz, Patrick Bindels, Alexandra Brandt-Kerkhof, Danielle van Diepen, Clemens Dirven, Tjebbe Galema, Jeanette Goudzwaard, Mieke Hazes, Sjoerd Lagarde, Harmke Polinder-Bos, Eva Maria Roes, Hanneke Takkenberg and Mark van Vledder for active participation in this research. 

We are grateful for Lisa Caulley for her suggestions to revise the final manuscript. We are grateful for H. Karreman and C. Van der Velden-van der Graaf for the work they have done for the quality of life data collection. Moreover, we want to thank Ruben Goedhart, Esther van Spronsen and Linda van der Sluijs – van der Beek for extracting the data from the electronic patient registry. 
...
There was no funding for this research and no conflicts of interest to report. 
