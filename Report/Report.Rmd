...
results_pooled$std <- 0
for(i in unique(results_pooled$Label)){
  results_pooled$std[results_pooled$Label==i] <- 
    mean(results_pooled$QALY_med[results_pooled$Label==i])
}

desc_df <- results_pooled[results_pooled$delay!=999,]

desc_df_steiler <- desc_df[desc_df$Label%in%c("Instable AP, CABG",
                                              "AAA, surgical repair", 
                                              "Carotid endarterectomy"),]

desc_df_vlakker <- desc_df[desc_df$Label%in%c("ASD, repair",
                                              "NSCLC, lobectomy",
                                              ...
                                              "Mammaca., mastectomy", 
                                              "Cholangeoca., resection", "MI bladder ca, cystectomy"
                                              ,
              "AVR, TAVI",                                "High-grade glioma, resection","PAD F3-4, bypass",
                                              "Penis ca., resection",
                                              "Liver metastasis colonca, resection"),]
desc_df_lin     <- desc_df[!desc_df$Label%in%c(desc_df_vlakker$Label, desc_df_steiler$Label),]


plot_descend_steiler <- ggplot(desc_df_steiler, 
...
plot_descend_vlakker <- ggplot(desc_df_vlakker, 
       aes(x = delay, y = QALY_med-std, col = Label))+
  geom_line(cex = 2) + theme_bw() + theme(legend.position = "bottom", text=element_text(size = 18))+
  geom_vline(xintercept = seq(12,42, by=10), lty=2)+
  labs(x="Delay (weeks)", y="QALYs - mean QALYs", col="Procedure")
...
Jan van Busschbach recommended the following visualisation of the procedures:
