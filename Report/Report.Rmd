...
# Plot the PSA input data without the treament effect and Time to no effect on QoL
...
# Save the PSA input data of all parameters 
ggsave(plot_psa(psa_df = param_psa), 
       device = "png", file = "../figures/main_results/psa_est_all.png", 
       width = 26, height = 30, units="cm")

# Save the PSA input data of each parameter separately
...
for(p in unique(bnft$Label)){
  ...
  # Calculate benefit 
  # defined as the difference between QALYs at immediate surgery vs no surgery
...
for(p in unique(urg$Label)){
  ...
  QALY52 <- df_res[df_res$Label == p & df_res$delay == 52, ]$QALY
  
  # Calculate slope
  urg[urg$Label == p & urg$type == "QALY", "urg"]    <- quantile(QALY2 - QALY52, probs = 0.5  )/50*52/12
  urg[urg$Label == p & urg$type == "QALY", "urg_lo"] <- quantile(QALY2 - QALY52, probs = 0.025)/50*52/12
  urg[urg$Label == p & urg$type == "QALY", "urg_hi"] <- quantile(QALY2 - QALY52, probs = 0.975)/50*52/12
  
  # LYs with minimal delay
  LY2   <- df_res[df_res$Label == p & df_res$delay == 2, ]$LY
  
  # QALYs with maximal delay
  LY52 <- df_res[df_res$Label == p & df_res$delay == 52, ]$LY
  
  # Calculate benefit
  urg[urg$Label == p & urg$type == "LY", "urg"]    <- quantile(LY2 - LY52, probs = 0.5  )/50*52/12
  urg[urg$Label == p & urg$type == "LY", "urg_lo"] <- quantile(LY2 - LY52, probs = 0.025)/50*52/12
  urg[urg$Label == p & urg$type == "LY", "urg_hi"] <- quantile(LY2 - LY52, probs = 0.975)/50*52/12
}

#Extract the right order of presentation
urg_qaly <- urg[urg$type == "QALY", ]

order_populations   <- as.character(urg_qaly$Label[order(urg_qaly$urg, decreasing = TRUE)]) # using the populations (patient & intervention)
order_patients      <- as.character(urg_qaly$Pop[order(urg_qaly$urg, decreasing = TRUE)]) # of patients 
order_interventions <- as.character(urg_qaly$Intervention[order(urg_qaly$urg, decreasing = TRUE)]) # of interventions
```
...
desc_df <- df_results_pooled[df_results_pooled$delay != 999, ]
...
plot_descend_steaper <- ggplot(desc_df_steaper, 
       aes(x = delay, y = QALY_med - std, col = Label)) +
  ...
  labs(x = "Delay (weeks)", y="QALYs - mean QALYs", col = "Procedure")
...
# Save the figures 
ggsave(plot_descend_flatter, filename = "../figures/main_results/descend_flatter.png", device = "png", 
       width = 15, height=10)
ggsave(plot_descend_steaper, filename = "../figures/main_results/descend_steaper.png", device = "png", 
       width = 15, height=10)
...
est_ci_QALY <- matrix(nrow = length(order_populations),
                      ncol = length(unique(df_results_pooled$delay)),
                      dimnames = list(order_populations, 
                                      unique(df_results_pooled$delay)))
# Loop over the diseases, and store the QALYs (est+95% CI) in the matrix
for(i in order_populations){
est_ci_QALY[i, ] <- paste(sprintf("%.2f",df_results_pooled$QALY_med[df_results_pooled$Label==i]),
                         " (",
                         sprintf("%.2f",df_results_pooled$QALY_lo[df_results_pooled$Label==i]),
                         " - ",
                         sprintf("%.2f",df_results_pooled$QALY_hi[df_results_pooled$Label==i]),
                         ")", sep=""
                         )
...
colnames(res_main) <- paste("QALY, delay =",unique(df_results_pooled$delay),"weeks")

# To store the urg in
res_main$`QALY/month` <- NA

# Obtain the AAC/max (estimate +95% CI) and store in the data.frame
for(i in order_populations){
res_main[i,"QALY/month"] <- paste(sprintf("%.2f",
                                             urg_qaly$urg[
                                               urg_qaly$Label==i]
                                             ),
                                     " (",
...
## part of QALY that is LY


bnft2$Bnft[bnft2$type == "QALY"]    <- -bnft2$Bnft[bnft2$type == "QALY"]
bnft2$Bnft_lo[bnft2$type == "QALY"] <- -bnft2$Bnft_lo[bnft2$type == "QALY"]
bnft2$Bnft_hi[bnft2$type == "QALY"] <- -bnft2$Bnft_hi[bnft2$type == "QALY"]

bnft2[bnft2$type == "QALY", c("Bnft","Bnft_lo","Bnft_hi")][bnft2[bnft2$type == "QALY", c("Bnft", "Bnft_lo", "Bnft_hi")] > 0] <- 0

bnft2[bnft2$type=="LY", c("Bnft", "Bnft_lo", "Bnft_hi")][bnft2[bnft2$type == "LY", c("Bnft", "Bnft_lo", "Bnft_hi")] < 0] <- 0

breaks <- -15:15
names(breaks) <- abs(breaks)
bnft2$type <- factor(bnft2$type, levels = c("QALY", "LY"))
bnft_v2 <- ggplot(bnft2, aes(x = Label, y = Bnft, ymin = Bnft_lo, ymax = Bnft_hi)) +
  ...
  labs(y = "Associated with surgery",x = "Surgery")
...
## QALYs are left, so need to be negative
urg2$urg[urg2$type=="QALY"]    <- -urg2$urg[urg2$type=="QALY"]
urg2$urg_lo[urg2$type=="QALY"] <- -urg2$urg_lo[urg2$type=="QALY"]
urg2$urg_hi[urg2$type=="QALY"] <- -urg2$urg_hi[urg2$type=="QALY"]

## Do not allow negative values (make them 0)
urg2[urg2$type=="QALY",c("urg","urg_lo","urg_hi")][urg2[urg2$type=="QALY",c("urg","urg_lo","urg_hi")]>0] <- 0
urg2[urg2$type=="LY",c("urg","urg_lo","urg_hi")][urg2[urg2$type=="LY",c("urg","urg_lo","urg_hi")]<0] <- 0

## To ensure that QALYs also have positive ticks (x-axis)
breaks <- seq(-0.40,0.40, by=0.06)
names(breaks) <- abs(breaks)

## To ensure that QALY = Left, and LY = right
urg2$type <- factor(urg2$type, levels=c("QALY","LY"))
...
urg_v2 <- ggplot(urg2, aes(x = Label, y = urg, ymin=urg_lo, ymax=urg_hi)) +
  ...
  facet_wrap_custom(~type, scales = "free_x", scale_overrides = list(
    scale_override(1, scale_y_continuous(limits = c(-0.40, 0),
                                         breaks = seq(from = -0.40, to = 0,by = 0.05),
                                         labels = seq(from = 0.40,  to = 0,by = -0.05), 
                     expand = c(0, 0))),
    scale_override(2, scale_y_continuous(limits = c(0, 0.40),
                                         breaks = seq(from = 0, to = 0.40,by = 0.05),
                                         labels = seq(from = 0,  to = 0.40,by = 0.05), 
                     expand = c(0, 0)))
  ))+
  coord_flip() +
  theme_bw() +
  theme(panel.spacing.x = unit(0, "pt"), 
        strip.background = element_rect(colour = "black"),
        text=element_text(size=18))+
  labs(y="Loss per month",x="Surgery")
