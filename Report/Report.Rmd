...
urg_v2 <- ggplot(urg2, aes(x = Label, y = urg, ymin=urg_lo, ymax=urg_hi)) +
  ...
  facet_wrap_custom(~type, scales = "free_x", scale_overrides = list(
    scale_override(1, scale_y_continuous(limits = c(-0.25, 0),
                                         ...
                                         labels = seq(from = 0.3,  to = 0,by = -0.05), 
                     expand = c(0, 0))),
...
tbl_cls <- CreateCatTable(vars = c("Evidence_type","Evidence_class"),
                          strata = "Param",
                          data = param, 
                          test = FALSE)
tbl_cls <- print(tbl_cls,printToggle=FALSE)  
  

kable(tbl_cls)
write.csv2(tbl_cls, file = "../figures/main_results/evidence_tbl.csv")
```
